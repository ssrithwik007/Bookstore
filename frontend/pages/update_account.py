...
if response.status_code == 200:
    ...
    with st.container(border=True):
        ...
        if "Email" in selected_options and not re.match(email_pattern, email):
        ...
        is_email_valid = ("Email" not in selected_options or (re.match(email_pattern, email)))
        ...
        if st.button("Update", disabled= not(is_pwd_valid and is_email_valid)):
            if "Password" in selected_options and len(password) < 8:
                ...
                if update_response.status_code == 200:
                    ...
                    # st.error(f"Failed to update details: {update_response.json().get('detail')}")
